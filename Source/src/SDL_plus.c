...
drawPanel(SDL_Surface *src, SDL_Surface *dst, pDesc *player)
{
	...
	for(i=0;i<1;i++) {
		...
		if(i) {
			a.x=SCREENW-35;
			writeNumber(src,dst,a.x-10-151,a.y-1,player[i].level+1,1);
			writeNumber(src,dst,a.x-15-151,a.y+14,player[i].score,6);
		} else {
			writeNumber(src,dst,a.x+b.w+2-160,a.y-1,player[i].level+1,1);
			writeNumber(src,dst,11-160,a.y+14,player[i].score,6);
		}
		...
		if(i) {
			b.x=139;
			a.x=SCREENW-(54+38)+(38-b.w)+160-151;
		} else {
			b.x=100;
			a.x=54+160-151;
		}
