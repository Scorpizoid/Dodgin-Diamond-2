...
drawHiscores(int max)
{
	...
	/* header */
	writeCString(gfx, screen, 80+160, 2, "the hall of fame", 1);

	for(i=0;i<max;i++) {
		writeNumber(gfx, screen, 10, 23+i*17, i+1, 2);
		writeCString(gfx, screen, 30+160, 20+i*17, hiscore[i].name, 0);
		writeCString(gfx, screen, 180+160, 20+i*17, "st",0);
		writeNumber(gfx, screen, 200, 23+i*17, hiscore[i].stage,2);
		writeCString(gfx, screen, 236+160, 20+i*17, "sc",0);
...
#endif
#if !JOYSTICK_DISABLED && !PLAYER_DISABLED
...
#if !PLAYER2_DISABLED && !CONFIGURE_DISABLED
...
#endif
#if PLAYER2_DISABLED && CONFIGURE_DISABLED
	writeCString(gfx, screen, 105, 67, "hall of fame", (option==2));
	writeCString(gfx, screen, 105, 94, "about", (option==3));
	writeCString(gfx, screen, 105, 111, "exit game", (option==4));
#endif
				...
				if(mevent.key.keysym.sym==SDLK_RETURN) {
					switch(option) {
						...
						case 2:
							...
							player[0].stage=0;//player[1].stage=0;
							return 1;
#endif
						case 2:
							if(!hiscores())
...
drawCredits()
{
	...
	writeCString(gfx, screen, 20, 50, "this is dd2 version 1.01 for s80.", 0);
	writeCString(gfx, screen, 20, 80, "authors", 1);
	writeCString(gfx, screen, 40, 105, "original dd2 by juan j. martinez", 0);
	writeCString(gfx, screen, 40, 130, "s80 version by sami vuori", 0);
	writeCString(gfx, screen, 20, 160, "thanks for playing...", 0);
